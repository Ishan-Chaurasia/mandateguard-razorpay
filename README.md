...
- **Real-time visibility without heavy infra.** Orchestrating a 50+ charge batch pipeline live in the UI required real-time visibility into each decision step, without pulling in external message queues or a heavy WebSocket server. Fix: used **Server-Sent Events (SSE) via Express** to stream stage-by-stage diagnostic logs to the React UI in real time, and Node's native standard-library SQLite engine (`DatabaseSync`) for a zero-setup, self-contained deployment that runs identically locally and on cloud hosts like Render.
- **Messy, inconsistent bank error codes risked dangerous AI retries.** Real UPI failure codes vary wildly across Indian banks — cryptic codes like `U30` alongside unstructured HDFC-style strings. Left unchecked, this risked the AI classifier hallucinating a retry decision on an ambiguous or misread code. Fix: AI is used **purely for diagnosis** — never for the final retry/no-retry call. Deterministic NPCI guardrails in code enforce the actual limits, guaranteeing zero over-limit retries regardless of what the classifier outputs. Retry timing was also aligned to Indian salary credit windows (1st–5th of the month) instead of blind immediate retries.
- **Non-deterministic seed made metrics look inconsistent.** `npm run seed` originally generated a fresh random dataset every run, so headline dashboard numbers (recovered revenue, uplift %, recovery rate) shifted between reloads — on one run the uplift card even showed `+₹0` before recalculating. Not a policy bug, but in a fintech context, numbers that shift between views undermine trust. Fix: pinned the seed so the dataset is deterministic across runs. The structural guarantees (0 NPCI violations, 0 revoked-mandate retries) held constant throughout, since those are enforced by hard gates, not computed statistically.
